...
def index():
    ...
    if search_genres:
        ...
        query += f' AND bg.genre_id IN ({placeholders})'
        params.extend(search_genres)
    
    # Фильтр по годам
    if search_years:
        placeholders = ','.join(['%s'] * len(search_years))
    ...
    query += ' GROUP BY b.id ORDER BY b.year DESC'
    ...
    # Получаем список уникальных годов для выпадающего списка
    years_list = []
    try:
        cursor = db.connection().cursor(dictionary=True)
        cursor.execute('SELECT DISTINCT year FROM books ORDER BY year DESC')
    ...
    return render_template(
        'index.html',
        books=books[PER_PAGE * (page - 1) : PER_PAGE * page],
        count=count,
        page=page,
        genres=get_genres(),
        years=years_list,
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for genre_id in genres:
                ...
                cursor.execute(query, (book_id['id'], genre_id,))
                db.connection().commit()
